...
public class FakeInventoryBigChest extends FakeInventoryChest {

    public FakeInventoryBigChest(InventoryHolder holder) {
        this(holder, null);
    }

    public FakeInventoryBigChest(InventoryHolder holder, String title) {
    ...
    private static byte[] getDoubleNbt(BlockVector3 pos, BlockVector3 pairPos, String name) {
        ...
        try {return NBTIO.write(tag, ByteOrder.LITTLE_ENDIAN, true);
        } catch (IOException e) {throw new RuntimeException("未能为箱子创建NBT");}
    }
    ...
    protected List<BlockVector3> onOpenBlock(Player who) {
        var blockPositionA = new BlockVector3((int) who.x, ((int) who.y) + 2, (int) who.z);
        var blockPositionB = blockPositionA.add(1, 0, 0);
